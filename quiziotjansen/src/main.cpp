...
#define WIFI_SSID "jann"
#define WIFI_PASSWORD "12345678"

#define MQTT_BROKER  "broker.emqx.io"
#define MQTT_TOPIC_PUBLISH_TEMPERATURE "esp32_jann/temperature"
#define MQTT_TOPIC_PUBLISH_HUMIDITY "esp32_jann/humidity"
#define MQTT_TOPIC_PUBLISH_LIGHT "esp32_jann/light"  
...
void taskTemperatureAndHumidityCheck(void *pvParameters) {
  while (true) {
    ...
    } else if(temperature > 28 && (humidity < 80 && humidity > 60)) {
      digitalWrite(LED_GREEN, LOW);
...
void taskLightCheck(void *pvParameters) {
  while (true) {
    if(lux > 400) {
      Serial.printf("Warning!! door open; Light: %.2f lux\n", lux);
    } else {
